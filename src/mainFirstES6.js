...
const Page = {
  ...
  WindowChanged() {
    ...
    for (let i = 0; i < Page.AreaArr.length; i++) {
      ...
      const newBottomLimit = Page.AreaArr[i].top + Page.AreaArr[i].H;
    ...
    const topPos = (bodyH - bottomLimit) / 2;
    let leftPos = bodyW / 2 - this.Game.W / 2;
    const rightOffset = bodyW - (leftPos + rightLimit) - this.unitSize * 0.5;
...
Page.Game = new DrawAreaObj(0, 0, 10, 20, function () {
  ...
  const uDrawSize = Page.unitSize - 2;
  let drawL, drawT;
...
Page.UpcomingA = new DrawAreaObj(10.5, 2.6, 2.5, 2.5, function () {
  ...
  if (pcA !== 0) {
    ...
    const avgT = uDrawSize * (totalT / countedT.length + 0.5);
    const offsetL = this.left + this.W / 2;
    const offsetT = this.top + this.H / 2;
    ...
    for (let j = 0; j < pcA.UO.arr.length; j++) {
      ...
      const drawT = Math.floor(offsetT - avgT + pcA.UO.arr[j].y * uDrawSize);
...
Page.UpcomingB = new DrawAreaObj(10.5, 5.2, 2.5, 2.5, function () {
  ...
  if (pcB !== 0) {
    ...
    for (let j = 0; j < pcB.UO.arr.length; j++) {
      ...
      const drawT = Math.floor(offsetT - avgT + pcB.UO.arr[j].y * uDrawSize);
...
Page.UpcomingC = new DrawAreaObj(10.5, 7.8, 2.5, 2.5, function () {
  ...
  if (pcC !== 0) {
    ...
    for (let j = 0; j < pcC.UO.arr.length; j++) {
      ...
      const drawT = Math.floor(offsetT - avgT + pcC.UO.arr[j].y * uDrawSize);
...
Page.ScoreBarHigh = new DrawAreaObj(10.5, 0, 4.5, 1, function () {
  ...
  const text = `00000000${GM.ScoreHigh}`.slice(-7);
  left = this.left + this.W - 4;
  top = this.top + Page.unitSize * 0.8;
  const size = Math.floor(Page.unitSize * 0.8) + 0.5;
...
const GM = {
  ...
  // Set up initial game var values
  Initialize() {
...
GM.PcObj = class {
  constructor(color, rotCount, units) {
    this.x = 5;
    this.y = 0;
    this.color = color;
    this.UO = {};
...
GM.Pc = {
  ...
  Freeze() {
    if (GM.IsAlive) {
      ...
      for (let i = 0; i < this.Cur.UO.arr.length; i++) {
        ...
        const staticY = this.Cur.y + this.Cur.UO.arr[i].y;
  ...
  CheckCollisions(doRot, offsetX, offsetY) {
    let unitArr;
    ...
    for (let i = 0; i < unitArr.length; i++) {
      ...
      const testY = this.Cur.y + unitArr[i].y + offsetY;
      const limitX = GM.StaticUnits.length;
      const limitY = GM.StaticUnits[0].length;
